...
export default function NewsPopup({ newsItems }: NewsPopupProps) {
  ...
  return (
    <AnimatePresence>
      {isVisible && newsItems.length > 0 && (
        <motion.div
          initial={{ opacity: 0, scale: 0.9, y: 50, x: "-50%", filter: 'blur(10px)' }}
          animate={{ opacity: 1, scale: 1, y: 0, x: "-50%", filter: 'blur(0px)' }}
          exit={{ opacity: 0, scale: 0.9, y: 20, x: "-50%", filter: 'blur(10px)', transition: { duration: 0.3 } }}
          transition={{ 
            type: "spring", 
            stiffness: 260, 
            damping: 20,
          ...
          className="fixed bottom-6 sm:bottom-8 left-1/2 z-[100] w-[92%] max-w-lg md:max-w-xl"
        >
