...
export default function Signup() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!userSnap.exists()) {
        await setDoc(userRef, {
          name: result.user.displayName || 'Anime Fan',
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center relative overflow-hidden py-12 px-4 sm:px-6">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-pink-600/10 blur-[150px] rounded-full pointer-events-none"></div>
      
      <div className="w-full max-w-md relative z-10 transition-transform duration-500 hover:scale-[1.01]">
        
        {/* Custom CSS Anime Sticker */}
        <div className="absolute -top-12 -left-6 w-28 h-28 md:w-32 md:h-32 animate-pulse flex items-center justify-center z-20 hover:scale-110 transition-transform" style={{ animationDuration: '4s' }}>
           <div className="w-full h-full bg-gradient-to-tr from-purple-500 to-pink-500 rounded-full flex flex-col items-center justify-center border-[3px] border-white shadow-[0_0_25px_rgba(168,85,247,0.6)] relative overflow-hidden transform -rotate-12">
             <Cat className="w-12 h-12 text-white mb-1" />
             <span className="text-[10px] font-black tracking-widest text-white">YAY!</span>
             <Sparkles className="w-5 h-5 text-yellow-300 absolute top-3 left-3 animate-pulse" />
             <Heart className="w-4 h-4 text-white absolute bottom-3 right-4 animate-pulse fill-white" />
           </div>
        </div>

        <div className="bg-gray-900/80 backdrop-blur-xl border border-gray-800 rounded-3xl p-6 sm:p-8 shadow-[0_0_40px_rgba(236,72,153,0.15)] animate-in fade-in slide-in-from-bottom-8 duration-700">
          
          <div className="text-center mb-8">
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight mb-2 flex items-center justify-center gap-2">
              CREATE <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-400">ACCOUNT</span> <Sparkles className="w-5 h-5 sm:w-6 sm:h-6 text-purple-400" />
            </h1>
            <p className="text-gray-400 text-sm sm:text-base">Join the AnimeVerse network today.</p>
          </div>
          ...
          <form onSubmit={handleSignup} noValidate className="space-y-4 sm:space-y-4">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full relative group overflow-hidden rounded-xl p-[1px] mt-8 block transform transition-all active:scale-95"
            >
              <span className="absolute inset-0 bg-gradient-to-r from-pink-500 via-purple-500 to-pink-500 opacity-70 group-hover:opacity-100 transition-opacity blur-sm"></span>
              <span className="absolute inset-0 bg-gradient-to-r from-pink-500 via-purple-500 to-pink-500 opacity-100"></span>
          ...
          <div className="mt-8 text-center pt-2">
            <p className="text-gray-400 text-sm">
              ...
              <Link href="/login" className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-400 hover:opacity-80 transition-opacity">
                Login
              </Link>
            </p>
          </div>
